...
import { importStatement, importThingPaths } from './src/templates';
...
if (!inputfile) {
    ...
    asts.forEach(async ([ { name }, rest ]: AST) => {
        ...
        const localImports = [ name, ...importsForFile(rest, allInterfaces) ];
        const libraryImports = [ importThingPaths ];
        const allImports = libraryImports.concat(importStatement(localImports, filename));

        fs.writeFileSync(filepath, '// This file is autogenerated\n');
        if (allImports.length) {
            fs.appendFileSync(filepath, allImports.join(''));
        }
