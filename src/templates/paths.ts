...
const setFnBody = (node: Field, rootType: string): string => {
    const generics = node.path.reduce((acc, n: Field) => (acc += `, "${n.name}"`), rootType).concat(`, "${node.name}"`);
    const path = node.path.map(q => q.name).concat(node.name);
    return `return mutIn<${generics}>(root, ${JSON.stringify(path)}, ${node.name});`;
}

export const setFn = (node: Field, rootType: string, functionName: string) => {
...
export const getFn = (node: Field, rootType: string, functionName: string): string => {
    const generics = node.path.reduce((acc, n: Field) => (acc += `, "${n.name}"`), rootType).concat(`, "${node.name}"`);
    const path = node.path.map(q => q.name).concat(node.name);
